...
void control3d::ControlSim3D(	unsigned int control_type,
...
void control3d::SetConParam(array<double>^ conparam)
{
	using namespace control3d::conparam;
...
void control3d::GenTWorld(double time,vector<double>& wref,vector<double>& wref1,vector<double>& wref2)
{
...
void control3d::GenTWorld34(double time,vector<double>& wref3,vector<double>& wref4)
{
	const double theta = OMG * time;
...
void control3d::GenRL(double time,vector<double>& wref,vector<double>& wref1,vector<double>& wref2)
{
	GenTLocal(time,wref,wref1,wref2);
}

void control3d::GenRL34(double time,vector<double>& wref3,vector<double>& wref4)
{
	GenTLocal34(time,wref3,wref4);
}

void CalcJ23(matrix<double>& J1,matrix<double>& J2,matrix<double>& J3,vector<double>& q,vector<double>& q1,vector<double>& q2,vector<double>& q3,vector<double>& q4)
{
	static matrix<double> oldJ1(5,5,0),oldJ2(5,5,0);
...
void control3d::GenRW(double time,vector<double>& wref,vector<double>& wref1,vector<double>& wref2)
{
	vector<double>	ref(NUM_DOF),ref1(ref),ref2(ref);
	matrix<double>	T(NUM_DOF,NUM_DOF),TT(NUM_DOF,2),J(T),J1(J);
...
void control3d::GenRW34(double time,vector<double>& wref3,vector<double>& wref4)
{
	vector<double>	ref(NUM_DOF),ref1(ref),ref2(ref),ref3(ref),ref4(ref);
	matrix<double>	J(NUM_DOF,NUM_DOF),J1(J),J2(J),J3(J);
...
void control3d::CalcConvertMatrixT(matrix<double>& T,matrix<double>& TT,vector<double>& q)
{
...
void control3d::CalcJ(matrix<double>& J,vector<double>& q)
{
...
void control3d::CalcJdot(matrix<double>& Jdot,vector<double>& q,vector<double>& qdot)
{
...
vector<double> control3d::CalcOrientation(vector<double>& q)
{
...
void control3d::CalcJo(matrix<double>& Jo,vector<double>& q)
{
	double tha1 = q(4),thc = q(3);
	double sa = sin(tha1),sc = sin(thc),ca = cos(tha1),cc = cos(thc);
...
void control3d::CalcJodot(matrix<double>& Jodot,vector<double>& q,vector<double>& qdot)
{
	//x y1 z c a1 
	//0 1  2 3 4 
...
void control3d::CalcFrenetFrame(double time,matrix<double>& F,matrix<double>& F1,matrix<double>& F2)
{
	using namespace numeric::ublas;

	vector<double>	w(NUM_DOF),w1(w),w2(w),w3(w),w4(w),
...
vector<double> control3d::CalcOrientationControlSignal(double time,vector<double>& pl,vector<double>& pldot,double& phi)
{
	using namespace control3d::conparam;
	...
	vector<double> prl(pl),prldot(prl),prl2dot(prl);
	...
	vector<double> O(Jo.size1()),Odot(O),thodot(Jo.size2());
	O = CalcOrientation(pl);
	thodot(0) = tha1dot;
	thodot(1) = thcdot;
	Odot = prod(Jo,thodot);

	vector<double>	Oref(O),Orefdot(Oref),Oref2dot(Oref),
...
void control3d::SetStateVal(vector<double>& pos,vector<double>& vel,vector<double>& acc,vector<double>& posall,vector<double>& velall,vector<double>& accall)
{
...
void control3d::LogingData(unsigned int step,array<double,2>^ datalog)
{
	using namespace conparam;
	unsigned int i,j;
...
vector<double> control3d::SyncCon(double time,vector<double>& posall,vector<double>& velall,vector<double>& pw,vector<double>& pwdot)
{
	using namespace control3d::conparam;

	static vector<double> oldes,oldesdot,oldf;
...
void control3d::IndependentCon(unsigned int max_step,bool enable_noise,bool enable_resolution,bool enable_sync,array<double,2>^ datalog)
{
	using namespace control3d::conparam;
	...
	for(step=0;step<max_step;step++)
	{
		...
		// 位置情報の変換行列の計算
		matrix<double> T(pw.size(),pw.size()),TT(pw.size(),2);
		CalcConvertMatrixT(T,TT,pw);
		// オフセットの用意
		vector<double> offset(TT.size2());
		...
		// 速度情報の変換行列の計算
		matrix<double> J(T),Jdot(J),invJ(J);
		CalcJ(J,pw);
		math::invert(J,invJ);
		...
		// loging data
		LogingData(step,datalog);
	}
}

void control3d::IndependentCon(unsigned int step,short AioId,short CntId,bool enable_sync,array<double,2>^ datalog)
{
	using namespace control3d::conparam;
...
void control3d::ContouringCon(unsigned int max_step,bool enable_noise,bool enable_resolution,bool enable_sync,array<double,2>^ datalog)
{
	using namespace control3d::conparam;
	...
	for(step=0;step<max_step;step++)
	{
		...
		// L-systemでの誤差
		vector<double> el(ew),eldot(el),el2dot(el);
		el = prl - pl;
		eldot = prldot - pldot;
		el2dot = prl2dot - pl2dot;

		// Calculation Frenet-Frame
		matrix<double> F(T),Fdot(F),F2dot(F),tF,tFdot,tF2dot;
		CalcFrenetFrame(time,F,Fdot,F2dot);	// get Frenet frame(current)
		tF = trans(F);
		tFdot = trans(Fdot);
		tF2dot = trans(F2dot);

		vector<double> eFdot(eF);
		eF = prod(tF,el);
...
void control3d::ContouringCon(unsigned int step,short AioId,short CntId,bool enable_sync,array<double,2>^ datalog)
{
	using namespace control3d::conparam;
...
void control3d::modContouringCon(unsigned int max_step,bool enable_noise,bool enable_resolution,bool enable_sync,array<double,2>^ datalog)
{
	using namespace control3d::conparam;
	...
	for(step=0;step<max_step;step++)
	{
		...
		vector<double> pral(pl),praldot(pral),pral2dot(pral);
		(*GenR)(time-td,pral,praldot,pral2dot);

		matrix<double> Fa(F),Fadot(Fa),Fa2dot(Fa),tFa,tFadot,tFa2dot;
		...
		matrix<double> Q(F),Qdot(Q),Q2dot(Q),S(Q.size1(),Q.size2(),0);
		S(1,1) = S(2,2) = S(3,3) = S(4,4) = 1;
		Q = prod(matrix<double>(prod(Fa,S)),tFa);
		Qdot = prod(matrix<double>(prod(Fadot,S)),tFa) + prod(matrix<double>(prod(Fa,S)),tFadot);
		Q2dot = prod(matrix<double>(prod(Fa2dot,S)),tFa) + 2*prod(matrix<double>(prod(Fadot,S)),tFadot) + prod(matrix<double>(prod(Fa,S)),tFa2dot);
		vector<double> prnl(pl),prnldot(prnl),prnl2dot(prnl);
		...
		vector<double> enl(el),enldot(enl);
		enl = prnl - pl;
		enldot = prnldot - pldot;

		vector<double> eFadot(eFa);
		eFa = prod(tFa,enl);
...
void control3d::modContouringCon(unsigned int step,short AioId,short CntId,bool enable_sync,array<double,2>^ datalog)
{
	using namespace control3d::conparam;
